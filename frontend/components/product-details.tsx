...
const ProductDetails: React.FC<ProductDetailsProps> = ({ product, currentUser, hasPreviousConversation }) => {
  ...
  const { mutate: addToWishlist, isPending: addToWishlistLoading } = useAddToWishlist({
    onSuccess: () => {
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center gap-4">
              <Avatar>
                <AvatarImage src={`https://avatar.io/${product?.seller.user.email}`} />
                <AvatarFallback>{product?.seller.user.name.charAt(0)}</AvatarFallback>
              </Avatar>
              <div>
                <p className="font-medium">{product?.seller.user.name}</p>
                <p className="text-gray-500 text-sm">{product?.seller.user.email}</p>
              </div>
            </div>
          </div>
        </CardContent>
        ...
        {product?.seller.user.id===currentUser?.id?(
            ...
            <CardFooter className="flex justify-end">
                <Button variant="default" className="mr-2" onClick={handleAddToWishlist} disabled={addToWishlistLoading}>
                ...
                <Button variant="secondary" onClick={()=>handleChat()}>Have doubts? Chat With the Seller!</Button>
            </CardFooter>
        )}
      </Card>
    </div>
