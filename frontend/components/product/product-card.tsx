  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      <div className="relative">
        ...
        />
        <Button onClick={handleWishlist} className="absolute bottom-3 right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-lg hover:bg-white">
          <Heart className="w-4 h-4 text-gray-600" fill={`${isInWishlist(product.id) ? "red" : "white"}`} stroke={`${isInWishlist(product.id) ? "red" : "black"}`} />
        </Button>
      </div>
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center justify-between text-sm text-gray-500 space-x-2">
            <div className="flex items-center  space-x-2">
                <span>{findYearDifference(product.seller.yearEstablished) || 0} yrs</span>
