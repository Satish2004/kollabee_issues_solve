    ...
    const fetchOnboardingData = async () => {
      try {
        const [sellers, buyers] = await Promise.all([
          AdminApi.getMonthlyOnboarding("seller"),
          AdminApi.getMonthlyOnboarding("buyer")
        ]);

        // Transform into chart-compatible format
        const mergedData = sellers.data.map((item: any, idx: number) => ({
          month: item.name,
          bulk: item.count, // sellers = bulk
          single: buyers.data[idx]?.count || 0 // buyers = single
        }));
...
function OrderAnalyticsChart({ data }: { data: { month: string; bulk: number; single: number }[] }) {
  return (
    <ChartContainer
      config={{
        bulk: {
