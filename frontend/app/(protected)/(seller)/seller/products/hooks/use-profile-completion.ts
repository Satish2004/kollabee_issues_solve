...
export function useProfileCompletion() {
    ...
    async function loadProfileCompletion() {
      try {
        ...
        if (Array.isArray(response)) {
          ...
          console.error("Invalid profile completion response format:", response)
          setProfileCompletion([])
        }
      } catch (err) {
        console.error("Failed to load profile completion:", err)
        setError(err instanceof Error ? err : new Error("Failed to load profile completion"))
        setProfileCompletion([])
      } finally {
        setIsLoading(false)
      }
  ...
  const remainingSteps = useMemo(() => {
    ...
    return Array.from({ length: 11 }, (_, i) => i + 1).filter((step) => !completedSet.has(step))
  }, [profileCompletion])
