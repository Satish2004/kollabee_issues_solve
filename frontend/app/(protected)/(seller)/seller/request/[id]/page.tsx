          ...
          <div className="border rounded-md mb-6">
            <div className="p-4">
              <div className="flex items-center mb-2">
                ...
                <span className="font-medium">{orderData.shippingAddress.fullName}</span>
              ...
              <div className="grid grid-cols-5 gap-4 mt-4">
                <div>
                  ...
                  <p className="font-medium text-sm">${orderData.totalAmount}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Country</p>
                  <p className="font-medium text-sm">{orderData.shippingAddress.country}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Quantity</p>
                  <p className="font-medium text-sm">{orderData.items[0]?.quantity} Units</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Status</p>
                  <div className="flex items-center">
                    <span className={`w-2 h-2 rounded-full ${orderData.isAccepted ? 'bg-green-500' : 'bg-yellow-500'} mr-2`}></span>
                    <p className="font-medium text-sm">{orderData.status}</p>
                  </div>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Price per unit</p>
                  <p className="font-medium text-sm">${orderData.items[0]?.price}</p>
                </div>
              </div>
            </div>
          </div>
