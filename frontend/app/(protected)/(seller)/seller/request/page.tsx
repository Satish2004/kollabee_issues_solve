...
//       setManufacturingRequests(manufacturingResponse.request || []);
//     } catch (error) {
...
const KollaBeeRequests = () => {
  ...
  const [currentManufacturingPage, setCurrentManufacturingPage] = useState(1);
  ...
  const paginate = (array: any[], page: number) =>
    array.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);

  const totalPagesRequests = Math.ceil(requests.length / ITEMS_PER_PAGE);
  const totalPagesManufacturing = Math.ceil(
    manufacturingRequests.length / ITEMS_PER_PAGE
  );
          ...
          <div className="p-2 sm:p-4 bg-white mt-3 sm:mt-6 rounded-xl">
            ...
            >
              ...
              {isLoading ? (
                ...
                <>
                  <div className="space-y-4 sm:space-y-8">
                    {paginate(requests, currentPage).map((request) => (
                      <RequestCard key={request.id} request={request} />
                    ))}
                  </div>
                  ...
                  <div className="flex justify-center mt-4 gap-2">
                    {Array.from({ length: totalPagesRequests }).map((_, i) => (
                      <button
                        key={i}
            ...
            >
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  {/* Pagination */}
                  <div className="flex justify-center mt-4 gap-2">
                    {Array.from({ length: totalPagesManufacturing }).map(
                      (_, i) => (
                        <button
                          key={i}
                          onClick={() => setCurrentManufacturingPage(i + 1)}
                          className={`px-3 py-1 rounded ${
                            currentManufacturingPage === i + 1
                              ? "bg-blue-600 text-white"
                              : "bg-gray-200"
                          }`}
                        >
