...
export function handleSocketConnection(socket: Socket, io: Server, prisma: PrismaClient) {
  ...
  socket.on("send_message", async (messageData) => {
    try {
      ...
      // Check if the conversation exists and its status
      ...
      // Check if the conversation is accepted or if the sender is the initiator (for the first message)
      const isInitiator = conversation.initiatedBy === senderId
      if (conversation.status !== "ACCEPTED" && !isInitiator) {
  ...
  socket.on("conversation_status_change", async ({ conversationId, status }) => {
    try {
      if (!conversationId || !status || !userId) {
        socket.emit("error", { message: "Missing required fields" })
        return
      }
      ...
      if (conversation.initiatedBy === userId && status === "ACCEPTED") {
        socket.emit("error", { message: "You cannot accept your own conversation request" })
        return
      }
