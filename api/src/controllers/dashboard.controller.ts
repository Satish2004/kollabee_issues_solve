...
export const getMetrics = async (req: any, res: Response) => {
  try {
    ...
    const requestsRevenueDifference = (totalRequestsRevenue._sum.targetPrice || 0) - (lastMonthTotalRequestsRevenue._sum.targetPrice || 0);
  ...
  try {
    ...
    if (!seller) return res.status(404).json({ error: "Seller not found" });
       ...
       switch (period) {
         ...
         case 'week':
           currentPeriodStart = new Date(currentDate);
           currentPeriodStart.setDate(currentDate.getDate() - currentDate.getDay()); // Start of current week (Sunday)
           currentPeriodStart.setHours(0, 0, 0, 0);
       ...
       const generateChartData = async (periodType: string) => {
         ...
         if (periodType === 'today') {
           ...
           for (let i = 0; i < weeksInMonth; i++) {
             const weekStart = new Date(currentDate.getFullYear(), currentDate.getMonth(), i * 7 + 1);
       ...
       res.json({
         ...
         metrics: {
           requests: {
             ...
             percentageChange: previousRequests === 0 
               ? currentRequests > 0 ? 100 : 0 
               : parseFloat((((currentRequests - previousRequests) / previousRequests) * 100).toFixed(2)),
           },
           messages,
           activeProducts,
         },
